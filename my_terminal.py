...
# Page Configuration
st.set_page_config(
    page_title="Advanced Option Chain & Signal Intelligence",
    layout="wide",
    initial_sidebar_state="expanded"
)

st.markdown("## 📊 Advanced F&O Option Chain & Signal Intelligence Dashboard")
st.markdown("---")
...
refresh_data = st.sidebar.button("🔄 Refresh Live Chain Data")

# 2. Unified Live API Data Fetching Function with Graceful Fallback
@st.cache_data(ttl=30)
def fetch_option_chain_data(symbol, expiry):
    """
    DhanHQ API v2 ya custom broker endpoint se live option chain fetch karne ka function.
    Agar credentials ya API connection fail hota hai, toh app crash hone ki bajaye 
    structure-testing ke liye intelligent fallback data generate karta hai.
    """
    try:
        # Agar secrets.toml mein credentials available hain toh live API call karein
        if "dhan" in st.secrets:
            ...
            payload = {"underlyingSymbol": symbol, "expiryDate": expiry}
            
            response = requests.post(url, json=payload, headers=headers, timeout=5)
            if response.status_code == 200:
                res_data = response.json()
                chain_list = res_data.get("data", [])
                spot_price = res_data.get("spotPrice", 0.0)
                if chain_list:
                    return pd.DataFrame(chain_list), spot_price

        # Fallback Mock Generator (Jab tak API credentials configure nahi hain, tab tak app smoothly chale)
        np.random.seed(42)
        strikes = np.arange(24000, 25500, 50) if "NIFTY" in symbol else np.arange(72000, 75000, 100)
        spot = float(strikes[len(strikes)//2] + 25)
        ...
        st.sidebar.warning(f"API Notice: Using simulation mode due to connection check.")
        return None, None

# Load Data
full_chain_df, spot_price = fetch_option_chain_data(selected_symbol, expiry_date)

if full_chain_df is None or full_chain_df.empty:
    st.error("⚠️ Option chain data load karne mein samasya aayi. Kripya apne parameters check karein.")
    st.stop()

df = full_chain_df.copy()

# 3. Calculations for Signals & Metrics
...
# 4. Automated Signal & Trend Metrics Box
st.markdown(f"### ⚡ Automated Market Signal & Trend Intelligence — `{selected_symbol}`")
...
# 5. Single-Page Navigation Tabs for Analytics
menu = st.radio(
