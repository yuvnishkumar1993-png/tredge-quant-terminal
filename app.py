...
if not st.session_state["password_correct"]:
    ...
    password_input = st.text_input("Enter Terminal Key", type="password", key="main_pass_input")
    if st.button("Access Terminal", key="access_btn"):
...
# ==============================================================================
# 4. MAIN DASHBOARD UI (STRICT UNIQUE KEYS)
# ==============================================================================
...
if selected_mode == "🌐 Direct NSE Live":
    ...
    with c1: 
        selected_symbol = st.selectbox("Choose Asset:", ["NIFTY", "BANKNIFTY", "FINNIFTY", "MIDCPNIFTY", "RELIANCE", "TCS", "INFY", "HDFCBANK", "SBIN"], key="nse_asset_sel")
    with c2: 
        active_lot = st.number_input("Lot Size:", value=DEFAULT_LOT_SIZES.get(selected_symbol, 65), key="nse_lot_inp")
    raw_df = fetch_live_nse(selected_symbol, active_lot)

# Mode 2: Dhan API
elif selected_mode == "⚡ Dhan Broker API":
    d1, d2 = st.columns(2)
    with d1: client_id = st.text_input("Dhan Client ID:", key="dhan_id_inp")
    with d2: access_token = st.text_input("Dhan Access Token:", type="password", key="dhan_tok_inp")
    c1, c2 = st.columns([2, 1])
    with c1: 
        selected_symbol = st.selectbox("Choose Asset:", ["NIFTY", "BANKNIFTY", "FINNIFTY", "MIDCPNIFTY", "SENSEX", "RELIANCE", "INFY"], key="dhan_asset_sel")
    with c2: 
        active_lot = st.number_input("Lot Size:", value=DEFAULT_LOT_SIZES.get(selected_symbol, 65), key="dhan_lot_inp")
    if client_id and access_token:
        raw_df = fetch_dhan(selected_symbol, client_id, access_token, active_lot)

# Mode 3: CSV Upload
else:
    uploaded_file = st.file_uploader("Upload Option Chain CSV File:", type=["csv"], key="csv_file_upl")
    active_lot = st.number_input("Fallback Lot Size:", value=65, key="csv_lot_inp")
