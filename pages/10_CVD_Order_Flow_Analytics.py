...
selected_symbol = st.sidebar.selectbox("Select Underlying Asset", all_symbols, index=0, key="cvd_bullet_sym")
...
lot_size = st.sidebar.number_input(
    ...
    step=1,
    key=f"cvd_bullet_lot_{selected_symbol}",
...
selected_expiry = st.sidebar.selectbox("Select Expiry Date", expiries, key="cvd_bullet_exp")

# --- 5-MINUTE AUTO-REFRESH CONFIGURATION ---
st.sidebar.markdown("---")
st.sidebar.markdown("### 🔄 Auto-Refresh Timer")
auto_refresh = st.sidebar.checkbox("Enable 5-Min Intraday Auto-Refresh", value=True, key="intraday_auto_ref_bullet")
...
# --- BULLETPROOF INTRADAY ENGINE WITH GRAVITY CENTER ---
@st.cache_data(ttl=30)
def fetch_bulletproof_cvd_gravity(c_id, token, sec_id, seg, exp, sym, lot):
    fallback_spot = 50500.0 if "BANK" in sym.upper() else (24500.0 if "NIFTY" in sym.upper() else 2500.0)
    
    df_out = pd.DataFrame()
    spot_val = fallback_spot
    gravity_center = fallback_spot

    if not c_id or not token: 
        return generate_mock_data(sym, fallback_spot)
...
def generate_mock_data(sym, fallback_spot):
    ...
    total_v = 0.0
    for s in strikes:
        d_val = np.random.uniform(-2000, 3000)
        cum_d += d_val
        c_v = np.random.uniform(1000, 5000)
        p_v = np.random.uniform(1000, 5000)
        comb = c_v + p_v
        total_w += s * comb
        total_v += comb
        mock_recs.append({
    ...
    df_m = pd.DataFrame(mock_recs)
    grav = round(total_w / total_v, 2) if total_v > 0 else spot
    return df_m, spot, grav

df_cvd, live_spot, gravity_center = fetch_bulletproof_cvd_gravity(client_id, access_token, resolved_sec_id, resolved_seg, selected_expiry, selected_symbol, lot_size)
...
# --- METRICS & SIGNALS ---
total_net_delta = disp_cvd['Volume Delta'].sum() if not disp_cvd.empty else 0.0
total_ce_vol = disp_cvd['CE Volume'].sum() if not disp_cvd.empty else 0.0
total_pe_vol = disp_cvd['PE Volume'].sum() if not disp_cvd.empty else 0.0
delta_imbalance_ratio = round((total_ce_vol - total_pe_vol) / (total_ce_vol + total_pe_vol + 1e-8) * 100.0, 2)

max_delta_row = disp_cvd.loc[disp_cvd['Volume Delta'].abs().idxmax()] if not disp_cvd.empty and 'Volume Delta' in disp_cvd else None
institutional_strike = int(max_delta_row['Strike']) if max_delta_row is not None and not disp_cvd.empty else live_spot
...
with tab1:
    ...
    if not disp_cvd.empty:
        ...
        st.warning("⚠️ प्रदर्शित करने के लिए पर्याप्त डेटा उपलब्ध नहीं है।")
