...
st.set_page_config(page_title="Institutional GEX & Gamma Flip Terminal", page_icon="🧲", layout="wide")
st.markdown("## 🧲 Master-Synced & Lot-Controlled GEX Terminal")
st.markdown("---")
...
tab1, tab2 = st.tabs(["📊 Single Asset GEX Profile", "⚡ F&O Stocks Master Gamma Flip Screener"])

# Standard Black-Scholes Gamma Engine
...
with tab1:
    ...
    selected_symbol = st.sidebar.selectbox("Select Underlying Asset", all_symbols, index=0, key="gex_sym_lot_synced")
    st.session_state.global_symbol = selected_symbol

    # Master Fetch (Pulls exact Lot Size, Security ID and Segment from CSV)
    resolved_sec_id, resolved_seg, master_lot = get_asset_details_from_master(selected_symbol)
    
    st.sidebar.markdown("---")
    st.sidebar.markdown("### ⚙️ Lot Size Control (Active Multiplier)")
    lot_size = st.sidebar.number_input(
        ...
        step=1,
        key=f"gex_lot_override_active_{selected_symbol}",
        help="मास्टर फाइल या गलत डेटा होने पर यहाँ से सही लॉट साइज़ सेट करें। कैलकुलेशन तुरंत अपडेट होगी।"
    )

    expiries = fetch_live_expiries(client_id, access_token, resolved_sec_id, resolved_seg)
    selected_expiry = st.sidebar.selectbox("Select Expiry Date", expiries, key="gex_exp_lot_synced")

    # --- CACHED FUNCTION WITH LOT SIZE PASSED DIRECTLY ---
    @st.cache_data(ttl=60)
    def fetch_lot_synced_gex(c_id, token, sec_id, seg, exp, lot):
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                for s_str, obj in oc_map.items():
                    ...
                    # DIRECT LOT SIZE MULTIPLIER APPLIED HERE
    ...
    # Passing `lot_size` directly into the function so cache invalidates on lot change
    gex_df, live_spot = fetch_lot_synced_gex(client_id, access_token, resolved_sec_id, resolved_seg, selected_expiry, lot_size)
    ...
    chart_range_mode = st.radio("Select Strike Span for GEX Chart:", ["±10 Strikes", "±20 Strikes", "All Strikes (Full Chain)"], horizontal=True, index=0, key="gex_radio_lot")
...
with tab2:
    ...
    scan_mode = st.radio("Select Screening Filter:", ["All F&O Stocks", "Negative GEX Zone Only", "Gamma Flip Zone Detected"], horizontal=True, key="scan_mode_gex_lot")

    @st.cache_data(ttl=300)
    def scan_market_gex_lot_synced(c_id, token):
        raw_symbols = get_available_symbols()
        exclude_indices = {"NIFTY", "BANKNIFTY", "FINNIFTY", "SENSEX", "MIDCAPNIFTY", "NIFTYNXT50"}
        symbols = [s for s in raw_symbols if s.upper() not in exclude_indices and not s.startswith("NIFTY") and not s.startswith("BANK")]
        if not symbols:
            symbols = ["RELIANCE", "TCS", "INFY", "SBIN", "HDFCBANK", "ICICIBANK", "AXISBANK", "TATAMOTORS"]
    ...
    df_scr = scan_market_gex_lot_synced(client_id, access_token)
