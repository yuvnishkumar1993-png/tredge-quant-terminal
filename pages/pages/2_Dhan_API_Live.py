...
st.set_page_config(
    page_title="Dhan API Live Engine - Tredge.in",
    page_icon="⚡",
    layout="wide"
)
...
st.title("⚡ Dhan Broker API Live Quant Engine")
st.caption("Real-Time Dhan Stream, Full Chain PCR, ATM ±10 Strikes, Change in OI & IV Skew Curve")

# Credentials Input Box
d1, d2 = st.columns(2)
with d1:
    client_id = st.text_input("Dhan Client ID:", type="default", placeholder="e.g. 1000123456")
with d2:
    access_token = st.text_input("Dhan Access Token:", type="password", placeholder="Enter Access Token")
...
# Dhan API Option Chain Fetcher
def fetch_dhan_option_chain(symbol, c_id, token, lot):
    try:
...
with c1:
    selected_symbol = st.selectbox(
        "Select Asset:", 
        ["NIFTY", "BANKNIFTY", "FINNIFTY", "MIDCPNIFTY", "SENSEX", "BANKEX", "RELIANCE", "TCS", "INFY", "HDFCBANK", "ICICIBANK", "SBIN"]
    )
with c2:
    active_lot = st.number_input("Lot Size:", value=DEFAULT_LOT_SIZES.get(selected_symbol, 65))

if client_id and access_token:
    raw_df = fetch_dhan_option_chain(selected_symbol, client_id, access_token, active_lot)
    ...
    if raw_df is not None and not raw_df.empty:
        ...
        with t1:
            ...
            fig_oi.update_layout(title=f"Dhan Live OI Walls (ATM ±10) - {selected_symbol}", barmode='group', template="plotly_dark", height=450)
        ...
        st.error("❌ Dhan API Response Error. Please check Client ID or Access Token.")
else:
    st.info("💡 कृपया Dhan Client ID और Access Token दर्ज करें।")
