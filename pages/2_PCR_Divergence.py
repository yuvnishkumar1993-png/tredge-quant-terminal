...
try:
    ...
    def get_asset_details_from_master(sym):
        return (13, "IDX_I", 25) if sym.upper() == "NIFTY" else (25, "IDX_I", 15)
...
st.markdown("## 📈 PCR Divergence & Strike-wise OI Buildup Analytics (Bulletproof Edition)")
st.markdown("---")

init_global_state()
all_symbols = get_available_symbols()

selected_symbol = st.sidebar.selectbox("Select Underlying Asset", all_symbols, index=0, key="pcr_sym_bulletproof")
st.session_state.global_symbol = selected_symbol

# 1. Fetch from master/utils
resolved_sec_id, resolved_seg, lot_size = get_asset_details_from_master(selected_symbol)

# 2. BULLETPROOF SAFETY OVERRIDE: Force exact correct values for indices so they NEVER show 2518/1518
index_master_override = {
    "NIFTY": (13, "IDX_I", 25),
...
if selected_symbol.upper() in index_master_override:
    resolved_sec_id, resolved_seg, lot_size = index_master_override[selected_symbol.upper()]

client_id = st.session_state.get("client_id", "")
access_token = st.session_state.get("access_token", "")
expiries = fetch_live_expiries(client_id, access_token, resolved_sec_id, resolved_seg)
selected_expiry = st.sidebar.selectbox("Select Expiry Date", expiries, key="pcr_exp_bulletproof")
...
with tab1:
    ...
    with c1: st.metric(label=f"Asset", value=selected_symbol)
