...
st.markdown("## 🧲 Master-Synced Standard Black-Scholes GEX & Gamma Flip Terminal")
st.markdown("---")
...
tab1, tab2 = st.tabs(["📊 Single Asset Master GEX Profile", "⚡ F&O Stocks Master Gamma Flip Screener"])
...
with tab1:
    st.sidebar.markdown("### ⚙️ Master GEX Desk Parameters")
    selected_symbol = st.sidebar.selectbox("Select Underlying Asset", all_symbols, index=0, key="gex_sym_master_synced")
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown("### ⚙️ Lot Size Control (Master Override)")
    lot_size = st.sidebar.number_input(
        ...
        step=1,
        key=f"gex_lot_override_master_{selected_symbol}",
        help="मास्टर फाइल से उठाया गया लॉट साइज़। जरूरत पड़ने पर यहाँ से बदल सकते हैं।"
    )

    expiries = fetch_live_expiries(client_id, access_token, resolved_sec_id, resolved_seg)
    selected_expiry = st.sidebar.selectbox("Select Expiry Date", expiries, key="gex_exp_master_synced")

    @st.cache_data(ttl=300)
    def fetch_master_synced_gex(c_id, token, sec_id, seg, exp, lot):
    ...
    gex_df, live_spot = fetch_master_synced_gex(client_id, access_token, resolved_sec_id, resolved_seg, selected_expiry, lot_size)
    ...
    chart_range_mode = st.radio("Select Strike Span for GEX Chart:", ["±10 Strikes", "±20 Strikes", "All Strikes (Full Chain)"], horizontal=True, index=0, key="gex_radio_master")
    ...
    with c4: st.metric(label="Master Lot Size", value=str(lot_size))
...
with tab2:
    ...
    scan_mode = st.radio("Select Screening Filter:", ["All F&O Stocks", "Negative GEX Zone Only", "Gamma Flip Zone Detected"], horizontal=True, key="scan_mode_gex_master")

    @st.cache_data(ttl=300)
    def scan_market_gex_stocks_only(c_id, token):
        # Fetch all symbols and strictly filter out indices (Nifty, BankNifty, Sensex, etc.)
        raw_symbols = get_available_symbols()
        exclude_indices = {"NIFTY", "BANKNIFTY", "FINNIFTY", "SENSEX", "MIDCAPNIFTY", "NIFTYNXT50"}
        
        # Keep only F&O Stocks (exclude indices)
        ...
        for sym in symbols[:20]: # Scan active F&O stocks
    ...
    df_scr = scan_market_gex_stocks_only(client_id, access_token)
